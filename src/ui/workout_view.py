...
class WorkoutView:
    ...
    def _initialize(self):
        ...
        back_button = ttk.Button(master=top_frame, text="< Back to List", command=self._handle_back)
        back_button.pack(side=constants.LEFT)

        date_label = ttk.Label(master=top_frame, text="Workout Date (YYYY-MM-DD HH:MM:SS):")
        date_label.pack(side=constants.LEFT, padx=(20, 5))
        date_entry = ttk.Entry(master=top_frame, textvariable=self._workout_date_var, width=25)
        date_entry.pack(side=constants.LEFT)

        save_workout_button = ttk.Button(master=top_frame, text="Save Workout", command=self._handle_save_workout)
        ...
        vsb = ttk.Scrollbar(master=tree_frame, orient="vertical", command=self._exercises_tree.yview)
        hsb = ttk.Scrollbar(master=tree_frame, orient="horizontal", command=self._exercises_tree.xview)
        self._exercises_tree.configure(yscrollcommand=vsb.set, xscrollcommand=hsb.set)
        ...
        add_exercise_button = ttk.Button(master=action_frame, text="Add Exercise", command=self._handle_add_exercise)
        edit_button = ttk.Button(master=action_frame, text="Edit Selected", command=self._handle_edit_selected)
        delete_button = ttk.Button(master=action_frame, text="Delete Selected", command=self._handle_delete_selected)
        add_set_button = ttk.Button(master=action_frame, text="Add Set to Selected Exercise", command=self._handle_add_set)
    ...
    def _load_workout_data(self):
        ...
        if self._workout_id:
            details = workout_service.get_full_workout_details(self._workout_id)
            if details and details['workout']:
                ...
                if details['exercises']:
                    for ex_detail in details['exercises']:
                        ...
                        if exercise.note:
                             self._exercises_tree.insert(ex_item_id, constants.END, text="Note", values=("Note", exercise.note, "", "", ""))
                        ...
                        if sets:
                            for set_instance in sets:
                                set_item_id = self._exercises_tree.insert(
                                    ...
                                    values=("Set", "", set_instance.set_number, set_instance.repetitions, set_instance.weight)
                                )
                                self._set_map[set_item_id] = set_instance.id
            else:
                 messagebox.showerror("Error", f"Could not load details for workout ID {self._workout_id}.")
                 self._handle_back()
        else:
    ...
    def _handle_save_workout(self):
        ...
        try:
            ...
            messagebox.showerror("Invalid Date", "enter the date in YYYY-MM-DD HH:MM:SS format.")
            return

        try:
            if self._workout_id:
                workout_service.update_workout_date(self._workout_id, date_str)
                messagebox.showinfo("Saved", "Workout date updated. Exercises/Sets are saved via their respective Add/Edit actions.")
            else:
                new_workout = workout_service.create_workout(date_str)
                self._workout_id = new_workout.id
                messagebox.showinfo("Created", f"New workout created with ID: {self._workout_id}. You can now add exercises and sets.")
    ...
    def _handle_add_exercise(self):
        if not self._workout_id:
             messagebox.showwarning("Save Workout First", "save the initial workout details (date) before adding exercises.")
             return

        dialog = EntryDialog(self._frame, "Add Exercise", {"Name": "", "Note (optional)": ""})
        if dialog.result:
             name = dialog.result.get("Name")
             note = dialog.result.get("Note (optional)")
             if name:
                 try:
                     exercise_service.add_exercise(self._workout_id, name, note if note else None)
                     self._load_workout_data()
                 except Exception as e:
                      messagebox.showerror("Error", f"Failed to add exercise: {e}")
             else:
                  messagebox.showwarning("Input Required", "Exercise name cannot be empty.")


    def _handle_add_set(self):
        selected_item = self._exercises_tree.focus()
        if not selected_item:
            messagebox.showwarning("Selection Required", "select an Exercise item in the tree to add a set to.")
            return
        ...
        if selected_item in self._exercise_map:
            ...
            dialog = EntryDialog(self._frame, f"Add Set {next_set_num}", {"Reps": "", "Weight (kg)": ""})
            if dialog.result:
                try:
                    ...
                    set_service.add_set(exercise_id, next_set_num, reps, weight)
                    self._load_workout_data()
                except ValueError:
                     messagebox.showerror("Invalid Input", "enter valid numbers (integer for Reps, number for Weight).")
                except Exception as e:
                     messagebox.showerror("Error", f"Failed to add set: {e}")
        else:
             messagebox.showwarning("Select Exercise", "select an Exercise item (not a Set or Note) to add a set to.")


    def _handle_edit_selected(self):
        selected_item = self._exercises_tree.focus()
        if not selected_item:
             messagebox.showwarning("Selection Required", "select an Exercise or Set to edit.")
             return
        ...
        if selected_item in self._exercise_map:
             ...
             if not exercise:
                 messagebox.showerror("Error", "Could not find selected exercise.")
                 self._load_workout_data()
                 return

             dialog = EntryDialog(self._frame, "Edit Exercise", {"Name": exercise.exercise_name, "Note": exercise.note if exercise.note else ""})
             if dialog.result:
                 name = dialog.result.get("Name")
                 note = dialog.result.get("Note")
                 if name:
                     try:
                         exercise_service.update_exercise(exercise_id, name, note if note else None)
                         self._load_workout_data()
                     except Exception as e:
                          messagebox.showerror("Error", f"Failed to update exercise: {e}")
                 else:
                      messagebox.showwarning("Input Required", "Exercise name cannot be empty.")
             ...
             dialog = EntryDialog(self._frame, f"Edit Set {set_instance.set_number}", {"Reps": str(set_instance.repetitions), "Weight (kg)": str(set_instance.weight)})
             if dialog.result:
                 try:
                     reps = int(dialog.result.get("Reps", 0))
                     weight = float(dialog.result.get("Weight (kg)", 0.0))
                     ...
                     set_service.update_set(set_id, set_instance.set_number, reps, weight)
                     self._load_workout_data()
                 except ValueError:
                      messagebox.showerror("Invalid Input", "enter valid numbers (integer for Reps, number for Weight).")
                 except Exception as e:
    ...
    def _handle_delete_selected(self):
        selected_item = self._exercises_tree.focus()
        if not selected_item:
             messagebox.showwarning("Selection Required", "select an Exercise or Set to delete.")
             return
        ...
        if selected_item in self._exercise_map:
             ...
             confirm = messagebox.askyesno("Confirm Delete", f"Delete Exercise '{name}' and ALL its sets?")
             if confirm:
                 try:
                     exercise_service.delete_exercise(exercise_id)
                     self._load_workout_data()
                 except Exception as e:
                      messagebox.showerror("Error", f"Failed to delete exercise: {e}")
             ...
             messagebox.showwarning("Invalid Selection", "Cannot delete this item type (e.g., Note). Select an Exercise or Set.")